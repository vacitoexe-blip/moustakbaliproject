...
const TradingSimulator = ({ onClose, user }) => {
  ...
  const [logs, setLogs] = useState([{ day: 1, text: 'Bienvenue dans le Simulateur de Trading. Vous avez 30 jours pour maximiser votre valeur nette.', type: 'info' }]);
  ...
  const handleNextDay = () => {
    if (day >= 30) {
      ...
      if (user && returnPct > 0) {
        ...
        toast.success(`Vous avez gagné ${rewardPoints} points et débloqué un défi !`);
      }
      return;
    }
    ...
    const newStocks = stocks.map(stock => {
      ...
      if (Math.random() < 0.15) {
        ...
        newLogs.push({ 
          day: day + 1, 
          text: `${stock.ticker}: ${isGood ? 'Rapport de bénéfices positif.' : 'Problème de chaîne d\'approvisionnement.'} Le prix a changé.`, 
  ...
  const buyStock = (ticker) => {
    ...
    if (cash >= stock.price) {
      ...
      setLogs(prev => [{ day, text: `1 action ${ticker} achetée à ${stock.price.toFixed(2)} $`, type: 'info' }, ...prev]);
    } else {
  ...
  const sellStock = (ticker) => {
    ...
    if (stock.shares > 0) {
      setCash(prev => prev + stock.price);
      setStocks(prev => prev.map(s => s.ticker === ticker ? { ...s, shares: s.shares - 1 } : s));
      setLogs(prev => [{ day, text: `1 action ${ticker} vendue à ${stock.price.toFixed(2)} $`, type: 'info' }, ...prev]);
    } else {
      toast.error('Aucune action à vendre');
    }
  };
  ...
  const resetGame = () => {
    ...
    setLogs([{ day: 1, text: 'Le jeu a redémarré.', type: 'info' }]);
    setGameOver(false);
    setHasStarted(false);
  };
    ...
    >
      ...
      {!hasStarted ? (
        <div className="flex-1 flex flex-col items-center justify-center text-center py-12 px-4 md:px-12">
          ...
          <h2 className="text-3xl md:text-4xl font-black text-white uppercase tracking-widest mb-4">Simulateur de Trading</h2>
        ...
        <>
          <div className="flex justify-between items-center mb-6 pr-8">
            <h2 className="text-xl md:text-2xl font-black text-white tracking-widest uppercase">Simulateur de Trading</h2>
            <div className="text-xs md:text-sm font-bold bg-white/10 px-3 py-1 rounded text-white uppercase tracking-wider">Jour {day} / 30</div>
          </div>
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
            <div className="bg-[#1a1a1a] p-3 md:p-4 rounded border border-white/5">
              <div className="text-[10px] md:text-xs text-white/50 uppercase tracking-widest mb-1">Liquidités</div>
              <div className="text-lg md:text-xl font-bold text-white">{cash.toFixed(2)} $</div>
            </div>
            <div className="bg-[#1a1a1a] p-3 md:p-4 rounded border border-white/5">
              <div className="text-[10px] md:text-xs text-white/50 uppercase tracking-widest mb-1">Portefeuille</div>
              <div className="text-lg md:text-xl font-bold text-white">{portfolioValue.toFixed(2)} $</div>
            </div>
            <div className="bg-[#1a1a1a] p-3 md:p-4 rounded border border-white/5">
              <div className="text-[10px] md:text-xs text-white/50 uppercase tracking-widest mb-1">Valeur Nette</div>
              <div className="text-lg md:text-xl font-bold text-white">{netWorth.toFixed(2)} $</div>
            </div>
            <div className="bg-[#1a1a1a] p-3 md:p-4 rounded border border-white/5">
              <div className="text-[10px] md:text-xs text-white/50 uppercase tracking-widest mb-1">Rendement</div>
            ...
            <table className="w-full text-left min-w-[600px]">
              <thead>
                <tr className="border-b border-white/10 text-[10px] text-white/40 uppercase tracking-widest bg-black/20">
                  <th className="py-3 px-4 w-1/3">Actif</th>
                  <th className="py-3 px-4 w-1/6">Prix</th>
                  <th className="py-3 px-4 w-1/6">Évolution</th>
                  <th className="py-3 px-4 w-16 text-center">Possédé</th>
          ...
          <div className="flex justify-center mb-6">
            ...
            >
              {day === 29 ? 'Terminer le Jeu' : 'Jour Suivant'}
            </button>
          </div>
        ...
        <div className="flex-1 flex flex-col items-center justify-center text-center py-16">
          <div className="text-xs font-black text-white/50 uppercase tracking-widest mb-4">Fin du Jeu - Jour 30</div>
          <h3 className="text-3xl md:text-5xl font-black text-white mb-6 leading-tight">
            Valeur Nette Finale :<br/>
          ...
          <div className={`text-xl font-bold mb-10 ${returnPct >= 0 ? 'text-green-500' : 'text-red-500'}`}>
            Rendement : {returnPct >= 0 ? '+' : ''}{returnPct.toFixed(2)} %
          </div>
          <button 
            onClick={resetGame} 
...
const BUDGET_CATEGORIES = [
  { id: 'rent', name: 'Loyer', limit: 900, fixed: true },
  { id: 'utilities', name: 'Factures', limit: 150, fixed: true },
  { id: 'groceries', name: 'Courses', limit: 400, fixed: false },
  { id: 'transport', name: 'Transport', limit: 200, fixed: false },
  { id: 'entertainment', name: 'Divertissement', limit: 200, fixed: false },
  { id: 'dining', name: 'Restaurants', limit: 200, fixed: false },
  { id: 'savings', name: 'Épargne', limit: 400, fixed: false },
  { id: 'misc', name: 'Divers', limit: 200, fixed: false }
];

const EVENTS = [
  {
    title: 'Voiture en panne',
    desc: 'Votre voiture nécessite des réparations immédiates.',
    choices: [
      { text: 'Payer 180 $ du Transport', cost: 180, category: 'transport', pts: 0 },
      { text: 'Payer 180 $ de l\'Épargne', cost: 180, category: 'savings', pts: 0 },
      { text: 'Ignorer les réparations', cost: 0, category: null, pts: -10 }
    ]
  },
  {
    title: "Dîner d'anniversaire",
    desc: 'Un ami proche vous a invité pour son anniversaire.',
    choices: [
      { text: 'Payer 65 $ des Restos', cost: 65, category: 'dining', pts: 0 },
      { text: 'Payer 65 $ du Divertissement', cost: 65, category: 'entertainment', pts: 0 },
      { text: 'Décliner l\'invitation', cost: 0, category: null, pts: -5 }
    ]
  },
  {
    title: 'Vente flash : Billets de concert',
    desc: 'Votre groupe préféré joue à proximité.',
    choices: [
      { text: 'Acheter 90 $ du Divertissement', cost: 90, category: 'entertainment', pts: 0 },
      { text: 'Ignorer', cost: 0, category: null, pts: 0 }
    ]
  },
  {
    title: 'Hausse des factures',
    desc: 'Une vague de froid a fait grimper votre facture de chauffage.',
    choices: [
      { text: 'Payer 40 $ des Factures', cost: 40, category: 'utilities', pts: 0 },
      { text: 'Payer 40 $ de Divers', cost: 40, category: 'misc', pts: 0 }
    ]
  }
];
...
const BudgetManager = ({ onClose, user }) => {
  ...
  const [logs, setLogs] = useState([{ week: 1, text: 'Le jeu a commencé. Les prélèvements automatiques de la Semaine 1 auront lieu au clic sur Semaine Suivante.', type: 'info' }]);
  ...
  const addSpend = (catId, amount, isEvent = false) => {
    setSpent(prev => {
      ...
      if (prev[catId] <= catObj.limit && newAmount > catObj.limit) {
        setScore(s => s - 3);
        setLogs(l => [{ week, text: `⚠️ Budget dépassé pour ${catObj.name} ! (-3 pts)`, type: 'error' }, ...l]);
      }
      return { ...prev, [catId]: newAmount };
    });
    if (!isEvent) {
      setLogs(l => [{ week, text: `Dépensé ${amount} $ pour ${BUDGET_CATEGORIES.find(c => c.id === catId).name}`, type: 'info' }, ...l]);
    }
  };
  ...
  const handleEventChoice = (choice) => {
    if (choice.cost > 0 && choice.category) {
      ...
      setLogs(l => [{ week, text: `Événement résolu : Payé ${choice.cost} $ de ${BUDGET_CATEGORIES.find(c => c.id === choice.category).name}`, type: 'warning' }, ...l]);
    }
    if (choice.pts < 0) {
      setScore(s => s + choice.pts);
      setLogs(l => [{ week, text: `Événement résolu : ${choice.text} (${choice.pts} pts)`, type: 'error' }, ...l]);
    }
    if (choice.cost === 0 && choice.pts === 0) {
      setLogs(l => [{ week, text: `Événement résolu : Ignoré.`, type: 'info' }, ...l]);
    }
  ...
  const handleNextWeek = () => {
    ...
    setLogs(l => [{ week: week + 1, text: `Prélèvement automatique du loyer (225 $) et des factures (37.5 $).`, type: 'info' }, ...l]);
  ...
  const resetGame = () => {
    ...
    setScore(100);
    setLogs([{ week: 1, text: 'Le jeu a redémarré.', type: 'info' }]);
    setCurrentEvent(null);
    setGameOver(false);
    setHasStarted(false);
  };
    ...
    >
      ...
      {!hasStarted ? (
        <div className="flex-1 flex flex-col items-center justify-center text-center py-12 px-4 md:px-12 overflow-y-auto">
          ...
          <h2 className="text-3xl md:text-4xl font-black text-white uppercase tracking-widest mb-4">Gestionnaire de Budget</h2>
        ...
        <div className="flex flex-col flex-1 overflow-hidden">
          <div className="flex justify-between items-center mb-6 pr-8">
            <h2 className="text-xl md:text-2xl font-black text-white tracking-widest uppercase">Gestionnaire de Budget</h2>
            <div className="text-xs md:text-sm font-bold bg-white/10 px-3 py-1 rounded text-white uppercase tracking-wider">Semaine {week} / 4</div>
          </div>
          ...
          {!gameOver ? (
            <div className="flex flex-col flex-1 overflow-hidden relative">
              ...
              <div className="grid grid-cols-2 md:grid-cols-4 gap-3 mb-6">
                <div className="bg-[#1a1a1a] p-3 rounded border border-white/5">
                  <div className="text-[10px] text-white/50 uppercase tracking-widest mb-1">Revenus</div>
                  <div className="text-lg font-bold text-white">3200 $</div>
                </div>
                <div className="bg-[#1a1a1a] p-3 rounded border border-white/5">
                  <div className="text-[10px] text-white/50 uppercase tracking-widest mb-1">Total Dépensé</div>
                  <div className="text-lg font-bold text-white">{totalSpent.toFixed(2)} $</div>
                </div>
                <div className="bg-[#1a1a1a] p-3 rounded border border-white/5">
                  <div className="text-[10px] text-white/50 uppercase tracking-widest mb-1">Restant</div>
                      ...
                      <div key={cat.id} className="flex flex-col md:flex-row md:items-center justify-between gap-2 border-b border-white/5 pb-2 last:border-0 last:pb-0">
                        <div className="w-full md:w-1/3">
                          <div className="flex justify-between text-xs font-bold text-white mb-1">
                            <span>{cat.name} {cat.fixed && '(Fixe)'}</span>
              ...
              <div className="flex justify-center mb-4">
                ...
                >
                  {week === 4 ? 'Terminer le Mois' : 'Semaine Suivante'}
                </button>
              </div>
              ...
              <div className="h-[100px] bg-black rounded border border-white/5 p-3 overflow-y-auto font-mono text-[10px] leading-relaxed">
                {logs.map((log, i) => (
                  <div key={i} className="mb-1">
                    <span className="text-white/30 mr-2">[Sem {log.week}]</span>
            ...
            <div className="flex-1 flex flex-col items-center justify-center text-center py-16">
              <div className="text-xs font-black text-white/50 uppercase tracking-widest mb-4">Mois Terminé</div>
              <h3 className="text-3xl md:text-5xl font-black text-white mb-4 leading-tight">
                Score Final : {score}/100
              </h3>
              <div className="text-lg text-white/80 mb-2">Total Épargné : <span className="font-bold text-green-400">{spent.savings.toFixed(2)} $</span></div>
              <div className={`text-lg font-bold mb-8 ${remaining >= 0 ? 'text-green-500' : 'text-red-500'}`}>
                Solde Restant : {remaining.toFixed(2)} $
              </div>
              <button 
                onClick={resetGame} 
                className="bg-white text-black px-8 py-3 font-black uppercase tracking-widest rounded hover:bg-gray-200 transition-colors"
