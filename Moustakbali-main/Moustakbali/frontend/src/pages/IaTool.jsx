...
import { Send, Bot, User, ArrowLeft, Sparkles, TrendingUp, ShieldCheck, Zap, MessageSquare, Square, Trash2, TrendingDown, PiggyBank, Building2, Coins, BarChart3, Wallet, BadgePercent, Landmark, Bitcoin, Home, Mic, MicOff, Volume2, VolumeX, Lock } from 'lucide-react';
...
const SUGGESTIONS = [
  ...
  { text: "Meilleur ETF pour débutants ?", icon: <Zap size={13} /> },
  { text: "كيف أوفر للتقاعد؟", icon: <PiggyBank size={13} /> },
  { text: "C'est quoi la Bourse de Casablanca ?", icon: <Landmark size={13} /> },
  { text: "Investir dans l'or ou les actions ?", icon: <Coins size={13} /> },
  { text: "Comment gérer son budget mensuel ?", icon: <Wallet size={13} /> },
  { text: "Qu'est-ce que l'intérêt composé ?", icon: <BadgePercent size={13} /> },
  ...
  { text: "Différence entre actions et obligations ?", icon: <BarChart3 size={13} /> },
  { text: "Comment calculer mon épargne retraite ?", icon: <PiggyBank size={13} /> },
  { text: "Impact de l'inflation sur l'épargne ?", icon: <TrendingDown size={13} /> },
...
const IaTool = () => {
  ...
  const isSubscribed = user && (user.plan === 'pro' || user.plan === 'iaplus');
  const isIAPlus = user && user.plan === 'iaplus';

  const [usage, setUsage] = useState(() => {
    const saved = localStorage.getItem('ia_tool_usage');
    if (saved) {
  ...
  useEffect(() => {
    localStorage.setItem('ia_tool_usage', JSON.stringify(usage));
  }, [usage]);
  ...
  const handleSend = async (text = input) => {
    ...
    if (!isSubscribed) {
      setMessages(prev => [...prev, { role: 'user', content: messageToSend }, { role: 'ai', content: "Vous ne pouvez pas utiliser l'IA car vous n'êtes pas abonné." }]);
      setInput('');
      return;
    }

    if (!isIAPlus && usage.count >= 5) {
      setMessages(prev => [...prev, { role: 'user', content: messageToSend }, { role: 'ai', content: "Vous avez atteint votre limite de 5 questions. Vous devez obtenir IA Plus pour continuer ou attendre 24h." }]);
      setInput('');
      return;
    }
    ...
    <div className="min-h-screen bg-bg-primary text-white font-sans">
      ...
      <div className="relative z-10 max-w-5xl mx-auto px-4 pb-8 flex flex-col" style={{ minHeight: 'calc(100vh - 80px)' }}>
        ...
        <div className="flex-1 flex flex-col bg-bg-card border border-border rounded-3xl overflow-hidden shadow-2xl" style={{ minHeight: '60vh' }}>
          ...
          {isSubscribed && (isIAPlus || usage.count < 5) && (
          ...
          {/* Input */}
          <div className="px-4 py-4 border-t border-border bg-bg-surface/30 relative">
            {!isSubscribed && (
              <div className="absolute inset-0 z-20 bg-bg-card/80 backdrop-blur-sm flex items-center justify-center rounded-b-3xl">
                <div className="bg-bg-surface border border-border px-6 py-3 rounded-2xl flex items-center gap-3 shadow-xl">
                  <Lock className="text-red-400" size={20} />
                  <p className="text-sm font-bold text-white">Vous ne pouvez pas utiliser l'IA car vous n'êtes pas abonné.</p>
                  <Link to="/payment" className="ml-4 px-4 py-1.5 bg-green-main text-black text-xs font-bold rounded-lg hover:bg-green-bright transition-colors">
                    S'abonner
                  </Link>
                </div>
              </div>
            ...
            {isSubscribed && !isIAPlus && usage.count >= 5 && (
              <div className="absolute inset-0 z-20 bg-bg-card/80 backdrop-blur-sm flex items-center justify-center rounded-b-3xl">
                <div className="bg-bg-surface border border-border px-6 py-4 rounded-2xl flex flex-col items-center gap-3 shadow-xl">
                  <Lock className="text-gold" size={24} />
                  <p className="text-sm font-bold text-white text-center">Limite atteinte. Vous devez obtenir IA Plus pour continuer ou attendre 24h.</p>
                  <Link to="/iaplus" className="mt-2 px-6 py-2 bg-gold text-black text-xs font-black uppercase tracking-widest rounded-xl hover:bg-white hover:scale-105 transition-all shadow-lg">
                    Obtenir IA Plus
                  </Link>
                </div>
              </div>
            ...
            <div className="flex items-center gap-2 bg-black/40 border border-border rounded-2xl px-3 py-2 focus-within:border-green-main/50 transition-all">
              ...
              <button
                ...
                disabled={!isSubscribed || (!isIAPlus && usage.count >= 5)}
              >
              ...
              <input
                ...
                disabled={(isLoading && !isRecording) || !isSubscribed || (!isIAPlus && usage.count >= 5)}
              ...
              {isLoading ? (
                ...
                <button
                  onClick={() => handleSend()}
                  disabled={!input.trim() || !isSubscribed || (!isIAPlus && usage.count >= 5)}
            ...
            <div className="flex justify-between items-center mt-2 px-2">
              ...
              {isSubscribed && (
                <p className={`text-[9px] font-bold uppercase tracking-widest ${!isIAPlus && usage.count >= 5 ? 'text-red-400' : 'text-green-main'}`}>
                  {isIAPlus ? 'IA Plus (Illimité)' : `${5 - usage.count} requêtes restantes (24h)`}
                </p>
              )}
            </div>
          </div>
