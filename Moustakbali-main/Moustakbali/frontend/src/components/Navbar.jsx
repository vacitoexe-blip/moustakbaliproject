...
import { Menu, X, Home, PieChart, BookOpen, Gamepad2, Award, LogOut, Ticket, Users, ArrowUpRight, LayoutDashboard, Bot, Sun, Moon } from 'lucide-react';
...
const Navbar = () => {
  ...
  const allNavLinks = [
    ...
    { name: 'Tableau de bord', path: '/dashboard', icon: <LayoutDashboard size={16} /> },
    { name: 'Analyses', path: '/analyses', icon: <PieChart size={16} /> },
    { name: 'Académie', path: '/academy', icon: <BookOpen size={16} /> },
  ...
  const navLinks = isLoggedIn 
    ? allNavLinks 
    : allNavLinks.filter(link => ['Accueil', 'À Propos'].includes(link.name));
    ...
    <div className="absolute top-8 left-0 w-full px-6 z-[100] pointer-events-none">
      <nav className="max-w-[1400px] mx-auto bg-black/40 backdrop-blur-2xl border border-white/10 rounded-full py-3 px-6 flex items-center justify-between shadow-2xl pointer-events-auto transition-all hover:border-white/20">
        ...
        <div className="flex items-center space-x-3 ml-4">
          ...
          <button
            ...
            className="p-2 rounded-full border border-white/10 hover:border-green-main/40 bg-white/5 hover:bg-green-main/10 text-white/60 hover:text-green-bright transition-all"
          >
      ...
      <div className="max-w-4xl mx-auto mt-3 px-6">
        <div className="bg-black/60 backdrop-blur-xl border border-white/5 rounded-full py-1.5 pl-2 pr-4 overflow-hidden shadow-xl flex items-center relative group">
          ...
          {/* Gradient Mask removed based on request */}
            ...
            >
              {[1, 2, 3, 4].map((_, i) => (
                <span key={i} className="text-[7px] md:text-[9px] font-black text-white/50 uppercase tracking-[0.2em] mx-8">
                  BTC: <span className="text-green-bright">$64,230 (+2.4%)</span> • S&P 500: <span className="text-white">5,210 (+0.5%)</span> • MASI: <span className="text-green-bright">13,420 (+1.2%)</span> • GOLD: <span className="text-alert-red">$2,340 (-0.1%)</span> • LA FED MAINTIENT SES TAUX • NOUVELLE RÉGLEMENTATION CRYPTO EN UE • RALLYE DES VALEURS TECHNOLOGIQUES • STABILISATION DES PRIX DU PÉTROLE • LE BOOM DE L'IA CONTINUE
                </span>
              ))}
            </motion.div>
          </div>
